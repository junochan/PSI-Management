...
**本技能中所有会改动远端状态或本地凭证的操作，必须先暂停 → 向用户确认 → 得到明确同意 → 才可执行。**

### 什么是写操作（必须确认）

| 类别 | 示例 |
|------|------|
| 登录 | `psims auth login`（写入本地 token 文件 `~/.psi-smart-ims/token`） |
| SSO登录 | `psims auth sso-login`（可选写 token 文件） |
| 登出 | `psims auth logout`（删除本地 token 文件） |
| 改密 | `POST /auth/change-password`（修改密码，不可逆） |

**可不经确认的只读操作**：`psims auth token-path`（查看路径）、`psims auth navigation`（拉取菜单/权限）等纯读。

### 确认流程

```
1. 收集必要参数（用户名/密码/token文件路径等）
2. ⚠️ 告知用户将写入/删除的 token 文件路径
3. ⛔ 停止，等待用户回复
4. 用户明确确认后才执行
```

**注意**：登录操作涉及密码传输，**严禁在回复中明文回显密码**。
