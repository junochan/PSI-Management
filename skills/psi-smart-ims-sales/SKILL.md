...
## ⚠️ 销售订单履约顺序（强制规则，不可跳过）

**销售订单的状态流转必须严格按以下顺序执行，不可跳步：**

```
创建销售订单 → ① 确认付款 → ② 确认发货 → ③ 确认收货
```

| 规则 | 说明 |
|------|------|
| **付款前置** | 创建销售订单后，**必须先确认付款**，才能执行发货操作 |
| **发货前置** | 确认付款完成后，**必须先确认发货**，才能执行收货操作 |
| **不可跳步** | 禁止未付款直接发货，禁止未发货直接收货 |

**执行流程：**
1. 创建销售订单后 → 检查 `payStatus`，若为 `unpaid`，**必须先执行付款**（`PUT /sales/orders/{id}/payment`）
2. 付款完成后 → 才能执行发货（`POST /sales/orders/{id}/shipping`）
3. 发货完成后 → 才能执行收货（`PUT /sales/orders/{id}/received`）

若用户要求在未完成前置步骤的情况下跳步操作（如直接发货、直接收货），**必须先拒绝并说明正确的履约顺序**，引导用户按流程执行。
...
步骤 4：创建成功后，提醒用户按履约顺序执行：先确认付款 → 再发货 → 最后确认收货
```
...
### 确认付款（PUT /sales/orders/{id}/payment）
...
步骤 4：付款成功后，提示用户下一步可执行发货操作
```

### 确认收货（PUT /sales/orders/{id}/received）

```
步骤 0：前置校验
  检查订单物流状态是否已发货（status 是否为 shipped）
  - 若未发货：必须先执行发货操作，禁止直接收货
  - 若已发货：继续下一步
...
步骤 4：收货成功后，提示用户订单已完成
```

### 取消（PUT /sales/orders/{id}/cancel）
...
> **注意**：已发货或已收货的订单通常不允许取消，需根据订单当前状态判断。

### 确认发货（POST /sales/orders/{id}/shipping）

```
步骤 0：前置校验
  检查订单 payStatus 是否为 paid
  - 若未付款（unpaid）：必须先执行付款操作，禁止直接发货
  - 若已付款：继续下一步
