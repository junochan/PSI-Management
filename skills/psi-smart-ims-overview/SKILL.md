...
description: >-
  智链进销存 smart-ims 全局说明：在 Linux 环境下使用 psims CLI（安装、全局参数）、JWT、API 基址、统一响应与技能索引。
...
CLI 子命令名与 `psi-manage-system-front/src/api/index.js` 中的封装一一对应；底层即对 `GET/POST/PUT/DELETE http(s)://<host>:8080/api/v1/...` 发请求。
...
| 技能目录 | 作用 |
|----------|------|
| `psi-smart-ims-auth` | 登录、登出、token 路径 |
| `psi-smart-ims-products` | 商品、分类、图片、以图搜图 |
| `psi-smart-ims-suppliers` | 供应商 |
...
| `psi-smart-ims-system` | 用户、角色权限、日志、工具 |

## OpenClaw 使用建议

1. 优先用 **`psims <模块> ...`**，少用手拼 curl。
2. 需要上传图片时用 `psims products upload-image --file <路径>`（multipart 已由 CLI 处理）。
