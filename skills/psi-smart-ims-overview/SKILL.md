...
**本技能中所有会改动进销存系统数据的操作，必须先暂停 → 向用户确认 → 得到明确同意 → 才可执行。**

### 什么是写操作（必须确认）

所有非幂等或会改数据的调用，包括各业务域技能的：
- **新增**：`create`（商品、客户、供应商、仓库、用户、角色、采购单、销售单、售后单、调拨单等）
- **修改**：`update`、`set-permissions`、`handle`、`confirm` 等
- **删除**：`delete`、`batch-delete`
- **状态流转**：`cancel`、`payment`、`shipping`、`received`、`close`、`inbound`、`outbound` 等
- **上传**：图片上传、头像上传、Excel 导入

**可不经确认的只读操作**：`list`/`get`/统计/`navigation`/`token-path`/`spec` 等纯查询。

### 确认流程（每次写操作前必须完整执行）

```
1. 收集必要参数（各子技能定义的必填字段）
...
凡是会修改进销存数据的命令都必须遵循以下流程：

1. **先识别必填字段**：根据对应子技能和命令参数，列出本次操作的必填项。
2. **缺字段先补齐**：若用户未提供完整必填信息，先引导用户逐项填写，禁止直接调用写接口。
3. **⛔ 输出确认卡片**：按上方「确认卡片模板」格式展示拟执行操作，然后停止等待。
4. **收到明确确认才执行**：仅当用户回复「确认」等肯定语后才发起写请求。
5. **高风险操作加强确认**：删除、批量删除、取消、关单、出库、扣减库存等不可逆操作，需再次强调影响范围。
